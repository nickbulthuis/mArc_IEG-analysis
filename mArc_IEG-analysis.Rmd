...
To start, load data from this study, located here: *[[link to come upon GEO release]]*

The data is available in raw FASTQ form, CellRanger-processed barcode/feature matrices, and filtered count matrices as H5 files.

The filtered data have been processed by CellBender (Fleming et al., 2023, *Nat. Methods*) to remove empty droplets and noisy background signal. This step was completed via Google Batch API using the [CellBender Terra WDL workflow](https://app.terra.bio/#workspaces/b8ec4cd9-568b-49cd-af75-67b054a3a9dd) due to limitations on compute power.

For CellBender, `fpr` = `0.01` (default), with `remove-background` applied to each sample individually. `output_filtered.h5` was saved per sample, removing background RNA and any droplet with \<50% likelihood of containing a nucleus.

This script begins by importing these filtered files, for further quality control steps next. To reproduce, adjust file paths below for your local environment.

```{r load}

#populate sample info, to be used for data loading (set manually)
  samples <- data.frame(file.path =   c("data/001_out_filtered.h5",
                                        "data/002_out_filtered.h5",
                                        "data/003_out_filtered.h5",
                                        "data/004_out_filtered.h5",
                                        "data/005_out_filtered.h5",
                                        "data/006_out_filtered.h5"),
...
#run through the sample df to load each one
  for (i in 1:dim(samples)[1]) {
    ...
    SampleList[[i]] <- Read_CellBender_h5_Mat(file_name = samples[i, "file.path"], feature_slot_name = "features",
...
First QC step: filter out any remaining droplets with low UMI counts (mostly redundant with CellBender, but nevertheless).
...
Last QC step: remove nuclei suspected of being doublets (two or more nuclei caught in same droplet).
...
#rename the clusters in an informed way
  HPC <- RenameIdents(HPC, "0" = "CA1", "1" = "DG", "2" = "parasubiculum I", "3" = "subiculum",
                      "4" = "inhibitory I", "5" = "CA3", "6" = "prosubiculum", "7" = "parasubiculum II",
                      "8" = "oligodendrocyte", "9" = "other glia", "10" = "inhibitory II", "11" = "inhibitory III",
                      "12" = "SFO", "13" = "microglia")
...
#reorder the types for nice plotting
  type.levels <- c("DG", "CA1", "CA3", "prosubiculum", "parasubiculum I", "parasubiculum II", "subiculum",
                   "inhibitory I", "inhibitory II", "inhibitory III", "SFO", "oligodendrocyte", "microglia", "other glia")
...
#generate a nice set of colors for use in plotting by cell type
  ...
  HPC@misc$type <- HPC@misc$type[c(6, 1, 3, 12, 11, 10, 9, 13, 14, 15, 21, 17, 18, 19)]
...
Generate new metadata (+/-) for *Arc* and *c-fos* expression, where count \> `0` is considered positive. Also assign IEG intersections, where every cell is assigned one of the following: *Arc-cfos-*, *Arc+cfos-*, *Arc-cfos+*, or *Arc+cfos+*. (Since we used CellBender on this data to remove background ambient RNA, we are comfortable using a threshold of `0` as a positive cutoff.)
...
#plot with nice legend and labels
  plot1 <- DimPlot(DG, group.by = "Arc.cfos", pt.size = 3, order = DG@misc$IEG.cla) +
                   ...
                   annotate("text", x = 5, y = -6.4, size = 10, label = paste0("n = ", glue({prettyNum(dim(DG)[2], big.mark = ",")}), " nuclei"))
  plot1 #print plot
  ...
  #plotting volcano plot
    ggplot(gene1.DEGs) + theme_cowplot() +
           ...
           geom_label_repel(data = gene1.DEGs.UP.top[1:num.top, ], aes(x = avg_log2FC, y = log10_padj, label = label), #add labels for top genes/unique top genes
                            size = 6.5, label.size = NA, fill = gene1.DEGs.UP.top$fill, color = gene1.DEGs.UP.top$textcolor,
...
#use function to generate plots: Arc+ vs - first
  plot4 <- intersect.volcano(gene1 = "Arc", gene2 = "Fos", gene1.DEGs = Arc.DEGs, gene2.DEGs = cfos.DEGs,
                             ...
                             num.top = 50, x.range = c(-3.5, 8), y.range = c(0, 350), repel = c(75, -2, 50, 25)) + theme(aspect.ratio = 1)
  plot4
...
#then c-fos+ vs -
  plot5 <- intersect.volcano(gene1 = "Fos", gene2 = "Arc", gene1.DEGs = cfos.DEGs, gene2.DEGs = Arc.DEGs,
                             ...
                             num.top = 50, x.range = c(-3.5, 7), y.range = c(0, 350), repel = c(75, -2, 50, 25)) + theme(aspect.ratio = 1)
  plot5
